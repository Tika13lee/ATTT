...
public class AES {
	...
	// chuyển chuỗi đầu vào thành ma trận 4x4
	...
	String[] keySchedule(String key) {
		...
		for (int i = 0; i < round; i++) {
			...
			keySchedule[i * 4 + 7] = xorArray(keySchedule[i * 4 + 3], keySchedule[i * 4 + 6]); // input to next round
																								// rotword
		}
	...
	// 3. MixColumn
	String mixor(int input1, int input2) {
		if (input1 == 1) {
	...
	String mixorInv(int input1, int input2) { // 9, 11, 13, 14
		...
		if (input1 == 11) { // ((((x * 2) * 2) + x) * 2) + x
			temp = xor(
					Integer.toBinaryString(binaryDoubler(Integer
							.parseInt(xor(Integer.toBinaryString(binaryDoubler(binaryDoubler(input2))), intToBin), 2))),
					intToBin);
		}
		if (input1 == 13) { // ((((x * 2) + x) * 2) * 2) + x
			temp = xor(
					Integer.toBinaryString(binaryDoubler(binaryDoubler(
							Integer.parseInt(xor(Integer.toBinaryString(binaryDoubler(input2)), intToBin), 2)))),
					intToBin);
		}
		if (input1 == 14) { // ((((x * 2) + x) * 2) + x) * 2
			temp = Integer.toBinaryString(binaryDoubler(Integer.parseInt(xor(
					Integer.toBinaryString(binaryDoubler(
							Integer.parseInt(xor(Integer.toBinaryString(binaryDoubler(input2)), intToBin), 2))),
					intToBin), 2)));
		}
		return temp;
	...
	// tạo vòng khóa
	void roundPrintCalculate(String array[], String array2[][], int multiplier, int blockStart) {
		int counter = 0;
		for (int i = 0; i < 4; i++) {
	...
	String[] roundInv(String keyBlock[], String roundKey[]) {
		int mixerInv[][] = { { 14, 11, 13, 9 }, { 9, 14, 11, 13 }, { 13, 9, 14, 11 }, { 11, 13, 9, 14 } };
	...
	String[] round(String keyBlock[], String roundKey[]) {
		...
		String mix[];
		String block[][];
		int mixer[][] = { { 2, 3, 1, 1 }, { 1, 2, 3, 1 }, { 1, 1, 2, 3 }, { 3, 1, 1, 2 } };
	...
	public String encrypt(String text, String key) {
		...
		String keyBlock[] = new String[16]; // 16 bytes
		...
		for (int i = 1; i < keySchedule.length / 16 - 1; i++) { // 9, 11, or 13 rounds
			...
			addRoundKey = round(keyBlock, addRoundKey);
		}
